...
class SentimentToolkit:
    ...
    def __init__(self):
        ...
        try:
            config = Config()
            if not config.gemini_api_key or not config.gemini_api_model:
                raise ValueError("Missing required configuration: GEMINI_API_KEY or GEMINI_API_MODEL")
                
            genai.configure(api_key=config.gemini_api_key)
            self.model = genai.GenerativeModel(config.gemini_api_model)
    ...
    async def _analyze_sentiment(self, user_thoughts: str) -> str:
        ...
        try:
            ...
            sentiment_response = self.model.generate_content(
                sentiment_prompt,
                generation_config={
                    "temperature": 0.2,
            ...
            if not sentiment_response or not sentiment_response.text:
                raise ValueError("Invalid sentiment analysis response")

            # Clean and validate emotion
            mood = sentiment_response.text.strip()
...
Generate a JSON response with this exact structure for grief support tools.
            ...
            tools_response = self.model.generate_content(
                tools_prompt,
                generation_config={
                    "temperature": 0.7,
            ...
            if not tools_response or not tools_response.text:
